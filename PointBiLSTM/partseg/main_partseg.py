...
import random
from models.PointBilstm_segg import Pointbilstm_partseg
...
def train(args, io):
    ...
    model = Pct().to(device)
    # model = PointxLSTM_Unet().to(device)
    # model = PointxLSTM_UNet().to(device)
...
def train_epoch(train_loader, model, opt, scheduler, epoch, num_part, num_classes, io):
    ...
    for batch_id, (points, label, target, norm_plt) in tqdm(enumerate(train_loader), total=len(train_loader), smoothing=0.9):
        ...
        # print(target)
        # print(target.shape)  # (32, 2048)
        # target: b,n
        seg_pred = model(points, norm_plt, to_categorical(label, num_classes))  # seg_pred: b,n,50 (32, 2048, 50)
        ...
        # print(1122222222222222222222222222222222222222222222222222)
        # print(batch_shapeious)
